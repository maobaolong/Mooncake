...
void MasterService::BatchEvict(double eviction_ratio) {
    ...
    for (size_t i = 0; i < metadata_shards_.size(); i++) {
        ...
        if (!candidates.empty()) {
            const long evict_num = std::min(ideal_evict_num, static_cast<long>(candidates.size()));
            if (evict_num > 0) {
                ...
                const auto target_timeout = candidates[evict_num - 1]->second.lease_timeout;
                long shard_evicted_count = 0;

                // Remove all expired objects with lease_timeout <= target_timeout
